...
# **通用函数加载不同数据集**
def load_imag_dataset(dataset, data_folder, batch_size, val, saved_tasks, get_subset=False):
    """
    根据数据集名称加载相应的数据集，并返回 dataloaders, task_split, num_classes, classes_per_task.
    如果 get_subset=True，则返回 subset_dataloaders
    """
    
    dataset_mapping = {
        "cifar50": (get_cifar_50, 50, 10),
        "5data": (get_5_dataset, 50, 10),
        "cifar100": (get_split_cifar100, 100, 5),
        "mnist": (get_split_mnist, 10, 2),
    }
    ...
    # 获取相应的数据加载函数和类别信息
    dataset_func, num_classes, classes_per_task = dataset_mapping[dataset]

    # 调用数据加载函数
    if get_subset:
        dataloaders, task_split, subset_dataloaders = dataset_func(
            data_folder, batch_size, val, saved_tasks=saved_tasks, get_subset=True
        )
    else:
        dataloaders, task_split = dataset_func(
            data_folder, batch_size, val, saved_tasks=saved_tasks, get_subset=False
        )
        subset_dataloaders = None  # 如果不启用子集加载，返回 None

    

    return dataloaders, task_split, num_classes, classes_per_task, subset_dataloaders
