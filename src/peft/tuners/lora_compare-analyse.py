...
log_level = 30  # 或者 logging.DEBUG（数值 10）
...
class LoraConfig(PeftConfig):
    ...
    # -----Nlora tian添加了额外的参数 ，使用lora可以用来删掉注释掉
    # modified. This argument represents the dim of the previous LoRA parameters.
    # r_sum: int = field(default=0)
    # modified. This arguments represents whether modules named of 'loranew_A/B' are saved independently, rather than being combined with "lora_A/B".
    # save_loranew: bool = field(default=False)
    # -----Nlora tian添加了额外的参数 ，使用lora可以用来删掉注释掉
...
class LoraModel(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def __init__(self, model, config, adapter_name):
        logger.info(f"Using Lora with config: model:{model}, config:{config},adapter_name:{adapter_name}")
        super().__init__()
        self.model = model
        ...
        logger.info(
            f"*********8Using Lora with config After add_adapter: self.model:{self.model},self.forward:{self.forward}")
        logger.info(
            f"*********8Using Lora with config After add_adapter: self.peft_config[adapter_name]:{self.peft_config[adapter_name]}, self.peft_config:{self.peft_config},self.forward:{self.forward}")
        trainable_params = [n for n, p in self.model.named_parameters() if p.requires_grad]
        logger.info(f'*********8Using Lora with config After add_adapter trainable_params:{trainable_params}')

    def add_adapter(self, adapter_name, config=None):
        logger.info(f"add_adapter with config:  config:{config},adapter_name:{adapter_name}")
        if config is not None:
            model_config = self.model.config.to_dict() if hasattr(
                self.model.config, "to_dict") else self.model.config
            config = self._prepare_lora_config(config, model_config)
            self.peft_config[adapter_name] = config
        logger.info(f"self._find_and_replace() :{adapter_name}")
        self._find_and_replace(adapter_name)
        if len(self.peft_config) > 1 and self.peft_config[adapter_name].bias != "none":
            raise ValueError(
                "LoraModel supports only 1 adapter with bias. When using multiple adapters, set bias to 'none' for all adapters."
            )
        logger.info(f"mark_only_lora_as_trainable()")
        trainable_params = [n for n, p in self.model.named_parameters() if p.requires_grad]
        logger.info(f"mark_only_lora_as_trainabletrainable_params:{trainable_params}")
        ...
        for key in key_list:
            if isinstance(lora_config.target_modules, str):
                target_module_found = re.fullmatch(
                    lora_config.target_modules, key)
            else:
                target_module_found = any(key.endswith(
                    target_key) for target_key in lora_config.target_modules)
            if target_module_found:
                ...
                logger.info(f"****debug loraModel---def _find_and_replace({adapter_name})--target:{target}")
                if isinstance(target, LoraLayer):
                    ...
                    if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
                        ...
                        logger.info(f"def _find_and_replace({adapter_name})--new_module:{new_module}")

                    logger.info(f"def _find_and_replace({adapter_name})--_replace_module:{new_module}")
    ...
    def merge_and_unload(self):
        ...
        if getattr(self.config, "model_type", None) == "gpt2":
            raise ValueError(
                "GPT2 models are not supported for merging LORA layers")
...
class LoraLayer:
    def __init__(
        ...
        # --------Nlora tian添加了这两句 ，使用lora可以用来删掉注释掉这两部分
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, r_sum):  # modified
        ...
        logger.info(
            f"update_layer with adapter_name:{adapter_name}, r:{r}, lora_alpha:{lora_alpha}, lora_dropout:{lora_dropout}, init_lora_weights:{init_lora_weights}")
        if r > 0:
            # 源码
            self.lora_A.update(nn.ModuleDict({adapter_name: nn.Linear(self.in_features, r, bias=False)}))
            self.lora_B.update(nn.ModuleDict({adapter_name: nn.Linear(r, self.out_features, bias=False)}))

            # 这一部分也是Nlora tian添加的，使用可以用来删掉注释掉这两部分
            # ----------------------------------------------------------------
            # self.loranew_A.update(nn.ModuleDict(
            #     {adapter_name: nn.Linear(self.in_features, r, bias=False)}))  # modified
            # self.loranew_B.update(nn.ModuleDict(
            #     {adapter_name: nn.Linear(r, self.out_features, bias=False)}))  # modified
    ...
    #             self.loranew_A[adapter_name].weight, a=math.sqrt(5))
...
class Embedding(nn.Embedding, LoraLayer):
    ...
    def __init__(
        ...
        LoraLayer.__init__(self, in_features=num_embeddings,
