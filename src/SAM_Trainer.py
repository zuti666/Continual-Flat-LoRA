...
class SAMContinualTrainer:
    def __init__(
        ...
        **kwargs  # 接收多余但不影响核心逻辑的参数
        ...
        if sam:
            ...
            ), torch.optim.SGD, rho=args.rho, lr=args.learning_rate)
        else:
        ...
        # 初始化方法
        if args.method == "er":
            self.algo = ER(args, num_classes)

        elif args.method == "agem":
            self.algo = AGEM(args, self.model, self.optimizer,
                             self.criterion, classes_per_task, len(dataloaders))
    ...
    def log_trainer_info(self):
        """输出 `Trainer` 相关的关键信息"""
        logger.info("🚀 SAMContinualTrainer 初始化完成！参数如下：")
        logger.info(f"📌 模型结构：{self.model.__class__.__name__}")
        logger.info(f"📌 当前设备：{self.device}")
        logger.info(f"📌 使用 SAM 优化器：{self.sam}")
        if self.sam:
            logger.info(f"📌 SAM ρ（rho）值：{getattr(self.args, 'rho', '未定义')}")

        logger.info(f"📌 学习率：{getattr(self.args, 'learning_rate', '未定义')}")

        logger.info(f"📌 优化器类型：{self.optimizer.__class__.__name__}")
        logger.info(f"📌 损失函数：{self.criterion.__class__.__name__}")

        logger.info(f"📌 当前任务类别数：{self.num_classes}")
        logger.info(f"📌 每个任务的类别数：{self.classes_per_task}")
        logger.info(
            f"📌 方法类型：{getattr(self.args, 'method', 'None')}, self.method:{self.method}")

        logger.info(f"📌 dataloaders 是否加载：{'是' if self.dataloaders else '否'}")
        logger.info(
            f"📌 subset_dataloaders 是否加载：{'是' if self.subset_dataloaders else '否'}")
        logger.info(f"📌 总任务数量：{len(self.task_split)}")
        logger.info(f"📌 task_split（前 3 个任务预览）：{self.task_split[:3]}")
        logger.info(f"📦 所有训练参数 args: {vars(self.args)}")
    ...
    def train_onlyonetask(self, taskID):
        ...
        logger.debug(f"只有一个任务 taskID :{taskID},self.args.num_train_epochs:{self.args.num_train_epochs}")
        if taskID != -1:
            task_id = taskID
            ...
            if self.args.save_task_models:
                logger.debug(f'Fininsh Training,Save the model in {self.args.output_dir}')
    ...
    def train_one_epoch(self, dataloader, task_id, subset_dataloader=None):
        ...
        logger.debug( f'train_one_epoch ,task_id:{task_id},self.method:{self.method} self.sam:{self.sam}')
        ...
        for X, y in iter(dataloader):
            ...
            if self.sam:
                ...
                if self.method == "er" and task_id > 0:
                    mem_pred = self.model(mem_x, None)
                    mem_pred = extract_logits(
                        mem_pred, mem_task_ids, self.classes_per_task, self.device
                    )
    ...
    def evaluate(self, max_task_id=None, logfile="log_eval_results.json"):
        ...
        with torch.no_grad():
            for eval_task_id in range(end_task_id):
                ...
                current_task_id = 0 if self.args.class_incremental else eval_task_id
        ...
        save_path = os.path.join(self.args.output_dir, logfile)
    ...
    def compute_loss_landscape_version2(
        ...
        def compute_loss(model):
            ...
            for i, batch in enumerate(test_loader):
                ...
                if self.method == "er" and eval_task_id > 0:
                    mem_x, mem_y, mem_task_ids = self.algo.sample(
                        self.batch_size, exclude_task=None, pr=False
                    )
        ...
        surf_file = os.path.join(output_dir, f"{save_file_name}_task{eval_task_id}_sharpness.h5")
        os.makedirs(output_dir, exist_ok=True)
        with h5py.File(surf_file, 'w') as f:
    ...
    def _sharpness_core(self, model, criterion_fn, A, epsilon=1e-3, p=0, bounds=None):
        ...
        flat_diffs = opt_result.x if A is 1 else A @ opt_result.x
    ...
    def _create_run_model(self, model, A, criterion_fn):
        ...
        def run_fn(delta):
            ...
            diff = delta if A is 1 else A @ delta
